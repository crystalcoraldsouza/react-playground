...
It is not mandatory to use redux for every application! You can manage with context and state just as well.

You cannot update a slice directly.

Write data:

> On an event you dispatch an action that calls a function (known as reducer) to modify the slice of the redux store.

Read data:

> Use a selector to read data from store which in turn modifies the react component. This is also known as subscribing to the store.

### Redux Toolkit

1. Install @reduxjs/toolkit and react-redux
2. Build our store
3. Connect out store to app
4. Create a slice
5. Dispatch an action
6. Read data with selector

### Major problems

> Selector is selecting a portion of the store to be subscribed to.

Subscribe to the right portion of the store, this can affect the performance majorly.
...
Both will work, but option 1 is is least efficient, if anything changes in the store, this component changes.

> reducers in a slice vs reducer in store.

In a store you have one `reducer` with multiple small reducers and one reducer is exported.

In a slice there multiple small reducer (reducer is exported) functions, hence `reducers`.

> Vanilla redux warning

There used to be a warning not to mutate the state and return of new state was mandatory.

This was not allowed! But now the toolkit allows it - redux does it internally.

```js
state.items.push(action.payload);
```

Redux uses Immer js - finds the diff and finds new state.

> Direct assignment of a value wont work
...
It doenst modify the state, you end up changing the reference to it.
State is a local variable, it wont chnage the actual original state.

```js
clearCart: (state) => {
    state.items.length = 0; // mutate the state
    // return { items:[] } // returns new state
},
```
